...
class SpecialClass(Base):
    ...
    dep_sem_id = Column(Integer, ForeignKey('dwarka.department_sem.id', ondelete='CASCADE'), nullable=False)
    course_id = Column(Integer, ForeignKey('dwarka.course.course_id', ondelete='CASCADE'), nullable=False)
    faculty_code = Column(String(50), ForeignKey('dwarka.faculty.faculty_code', ondelete='CASCADE'), nullable=False)
    room_id = Column(Integer, ForeignKey('dwarka.rooms.room_id', ondelete='CASCADE'), nullable=False)

    day_of_week = Column(Integer, CheckConstraint('day_of_week BETWEEN 1 AND 7'), nullable=False)
    slot = Column(Integer, nullable=False)  # e.g., 1-6 slots
